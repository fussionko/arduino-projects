...
void readP()
{
  mapX = map(analogRead(VRx), 0, 1023, 0, 1023);
  mapY = map(analogRead(VRy), 0, 1023, 0, 1023);
}

void dir()
{
  if (mapX >= 490 && mapX <= 530 && mapY <= 30 && mapY >= 0)
    smer_j = 1; //dol

  if (mapX >= 490 && mapX <= 530 && mapY <= 1023 && mapY >= 1000)
    smer_j = 2; //gor

  if (mapX >= 0 && mapX <= 30 && mapY <= 530 && mapY >= 490)
    smer_j = 3; //levo

  if (mapX >= 1000 && mapX <= 1023 && mapY <= 530 && mapY >= 490)
    smer_j = 4; //desno
}
...
void loop() {
  ...
  switch (smer_j)
  {
    case (3): if (checkPrevSmer())
        decrease();
      else
        smer_j = 0; break;
    case (4): if (checkPrevSmer())
        increase(); 
      else
        smer_j = 0;break;
  }
